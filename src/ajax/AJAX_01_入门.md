...
1. 什么是 AJAX ? [mdn](https://developer.mozilla.org/zh-CN/docs/Web/Guide/AJAX/Getting_Started)
   ...
   * 浏览器网页中，使用 AJAX技术（XHR对象）发起获取省份列表数据的请求，服务器代码响应准备好的省份列表数据给前端，前端拿到数据数组以后，展示到网页
     ...
     ![image-20230403173156484](AJAX_01_入门/image-20230403173156484.png)
...
5. 需求：从服务器获取省份列表数据，展示到页面上（体验 axios 语法的使用）
   ...
   * 完成效果：
     ...
     ![image-20230220113157010](AJAX_01_入门/image-20230220113157010.png)
...
2. 什么是 URL ？
   ...
   * 统一资源定位符，简称网址，用于定位网络中的资源（资源指的是：网页，图片，数据，视频，音频等等）
     ...
     ![image-20230403185206384](AJAX_01_入门/image-20230403185206384.png)

3. URL 的组成?

   * 协议，域名，资源路径（URL 组成有很多部分，我们先掌握这3个重要的部分即可）

     ![image-20230403185305934](AJAX_01_入门/image-20230403185305934.png)
...
4. 什么是 http 协议 ?
   ...
   * 叫超文本传输协议，规定了浏览器和服务器传递数据的格式（而格式具体有哪些稍后我们就会学到）
     ...
     ![image-20230403185356997](AJAX_01_入门/image-20230403185356997.png)

5. 什么是域名 ?

   * 标记服务器在互联网当中的方位，网络中有很多服务器，你想访问哪一台，就需要知道它的域名才可以

     ![image-20230403185406674](AJAX_01_入门/image-20230403185406674.png)

6. 什么是资源路径 ?

   * 一个服务器内有多个资源，用于标识你要访问的资源具体的位置

     ![image-20230403185428276](AJAX_01_入门/image-20230403185428276.png)

7. 接下来做个需求，访问新闻列表的 URL 网址，打印新闻数据

   * 效果图如下：

   ![image-20230220122455915](AJAX_01_入门/image-20230220122455915.png)
...
1. 什么是查询参数 ?
   ...
   * 举例：查询河北省下属的城市列表，需要先把河北省传递给服务器
     ...
     ![image-20230404101257205](AJAX_01_入门/image-20230404101257205.png)
...
1. 需求：根据输入的省份名字和城市名字，查询下属地区列表
   ...
   * 完成效果如下：
     ...
     ![image-20230220125428695](AJAX_01_入门/image-20230220125428695.png)
...
1. 想要提交数据，先来了解什么是请求方法
   ...
   * 请求方法是一些固定单词的英文，例如：GET，POST，PUT，DELETE，PATCH（这些都是http协议规定的），每个单词对应一种对服务器资源要执行的操作
     ...
     ![image-20230220130833363](AJAX_01_入门/image-20230220130833363.png)

     ![image-20230404104319428](AJAX_01_入门/image-20230404104319428.png)
...
2. 什么时候进行数据提交呢？
   ...
   * 例如：多端要查看同一份订单数据，或者使用同一个账号进行登录，那订单/用户名+密码，就需要保存在服务器上，随时随地进行访问
     ...
     ![image-20230404104328384](AJAX_01_入门/image-20230404104328384.png)

     ![image-20230404104333584](AJAX_01_入门/image-20230404104333584.png)
...
4. 需求：注册账号，提交用户名和密码到服务器保存
   ...
   ![image-20230404104350387](AJAX_01_入门/image-20230404104350387.png)
...
1. 如果注册相同的用户名，则会遇到注册失败的请求，也就是 axios 请求响应失败了，你会在控制台看到如图的错误：
   ...
   ![image-20230220131753051](AJAX_01_入门/image-20230220131753051.png)
...
4. 需求：再次重复注册相同用户名，提示用户注册失败的原因
   ...
   ![image-20230404104440224](AJAX_01_入门/image-20230404104440224.png)

   ![image-20230404104447501](AJAX_01_入门/image-20230404104447501.png)
...
2. 请求报文：是浏览器按照协议规定发送给服务器的内容，例如刚刚注册用户时，发起的请求报文：
   ...
   ![image-20230404104508764](AJAX_01_入门/image-20230404104508764.png)

   ![image-20230220132229960](AJAX_01_入门/image-20230220132229960.png)
...
3. 通过 Chrome 的网络面板如何查看请求体？
   ...
   ![image-20230220132617016](AJAX_01_入门/image-20230220132617016.png)
...
1. 响应报文：是服务器按照协议固定的格式，返回给浏览器的内容
   ...
   ![image-20230404104556531](AJAX_01_入门/image-20230404104556531.png)

   ![image-20230220133141151](AJAX_01_入门/image-20230220133141151.png)
...
3. HTTP 响应状态码：
   ...
   * 例如：404（客户端要找的资源，在服务器上不存在）
     ...
     ![image-20230220133344116](AJAX_01_入门/image-20230220133344116.png)
...
3. 例如：获取城市列表接口样子
   ...
   ![image-20230404104720587](AJAX_01_入门/image-20230404104720587.png)
...
2. 分析实现的步骤
   ...
   3. 提示信息，反馈给用户（这节课先来完成前 2 个步骤）
      ...
      ![image-20230404104851497](AJAX_01_入门/image-20230404104851497.png)
...
1. 需求：使用提前准备好的提示框，来把登录成功/失败结果提示给用户
   ...
   ![image-20230404104955330](AJAX_01_入门/image-20230404104955330.png)

   ![image-20230404105003019](AJAX_01_入门/image-20230404105003019.png)
...
1. 我们前面收集表单元素的值，是一个个标签获取的
   ...
   ![image-20230404105134538](AJAX_01_入门/image-20230404105134538.png)

2. 如果一套表单里有很多很多表单元素，如何一次性快速收集出来呢？

   ![image-20230404105141226](AJAX_01_入门/image-20230404105141226.png)
