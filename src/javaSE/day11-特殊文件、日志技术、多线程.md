...
![1667991441046](day11-特殊文件、日志技术、多线程/1667991441046.png)
...
![1667992083258](day11-特殊文件、日志技术、多线程/1667992083258.png)
...
![1667992486134](day11-特殊文件、日志技术、多线程/1667992486134.png)
...
![1667993320872](day11-特殊文件、日志技术、多线程/1667993320872.png)
...
![1667993682237](day11-特殊文件、日志技术、多线程/1667993682237.png)
...
![1667993581146](day11-特殊文件、日志技术、多线程/1667993581146.png)
...
![1667993965682](day11-特殊文件、日志技术、多线程/1667993965682.png)

**接下，同学们可以跟着步骤新建一个XML文件，试试！**

![1667994441934](day11-特殊文件、日志技术、多线程/1667994441934.png)
...
![1667995490654](day11-特殊文件、日志技术、多线程/1667995490654.png)
...
![1667996374837](day11-特殊文件、日志技术、多线程/1667996374837.png)

由于DOM4J是第三方提供的，所以需要把第三方提供的Jar包导入到自己的项目中来，才可以使用。具体步骤如下：

![1667996538290](day11-特殊文件、日志技术、多线程/1667996538290.png)
...
![1667996650787](day11-特殊文件、日志技术、多线程/1667996650787.png)

DOM4J解析XML需要用到的方法如下图所示

![1667996750188](day11-特殊文件、日志技术、多线程/1667996750188.png)
...
![1667996995326](day11-特殊文件、日志技术、多线程/1667996995326.png) 

### 2.3 XML解析2

获取到XML文件的根元素之后，接下来，就可以用根元素在获取到它里面的子元素（包括子标签、表属性等）。需要用到的方法如下图所示

![1667997211327](day11-特殊文件、日志技术、多线程/1667997211327.png)
...
![1668001422123](day11-特殊文件、日志技术、多线程/1668001422123.png)
...
- DTD约束案例
  ...
  ![1668001621567](day11-特殊文件、日志技术、多线程/1668001621567.png)
...
- Schame约束案例
  ...
  ![1668001745089](day11-特殊文件、日志技术、多线程/1668001745089.png)
...
![1668044513873](day11-特殊文件、日志技术、多线程/1668044513873.png)

**这里推荐同学们使用Logback日志框架，也在行业中最为广泛使用的。**

**Logback日志分为哪几个模块**

![1668044711404](day11-特殊文件、日志技术、多线程/1668044711404.png)
...
![1668045391911](day11-特殊文件、日志技术、多线程/1668045391911.png)

同时在文件中，也有一份这样的日志信息。文件在哪里内，从配置文件中去找

![1668045471304](day11-特殊文件、日志技术、多线程/1668045471304.png)

打开`D:/log/itheima-data.log`看一下文件中是否有记录日志吧！！

![1668045530645](day11-特殊文件、日志技术、多线程/1668045530645.png)
...
![1668045955362](day11-特殊文件、日志技术、多线程/1668045955362.png)

- **如下图所示，控制打开和关闭日志**

![1668046078574](day11-特殊文件、日志技术、多线程/1668046078574.png)

- **如下图所示，控制日志的输出的格式**

  日志格式是由一些特殊的符号组成，可以根据需要删减不想看到的部分。比如不想看到线程名那就不要[%thread]。但是不建议同学们更改这些格式，因为这些都是日志很基本的信息。

![1668046216355](day11-特殊文件、日志技术、多线程/1668046216355.png)

### 3.5 配置日志级别

![1668046420402](day11-特殊文件、日志技术、多线程/1668046420402.png)

- 在哪里配置日志级别呢？如下图所示

![1668046551345](day11-特殊文件、日志技术、多线程/1668046551345.png)
...
![1668046984412](day11-特殊文件、日志技术、多线程/1668046984412.png)
...
![1668047091631](day11-特殊文件、日志技术、多线程/1668047091631.png)
...
![1668047848218](day11-特殊文件、日志技术、多线程/1668047848218.png)

**最后我们还需要注意一点**：不能直接去调用run方法，如果直接调用run方法就不认为是一条线程启动了，而是把Thread当做一个普通对象，此时run方法中的执行的代码会成为主线程的一部分。此时执行结果是这样的。

![1668048108548](day11-特殊文件、日志技术、多线程/1668048108548.png)
...
```java
必须练习:
	1.编写一个Properties文件,并且使用Properties属性集类读取properties文件中的所有键值对数据
	2.使用Dom4j框架解析资料中的helloworld.xml文件---读取该xml文件中的所有数据
	3.根据dtd,schema约束编写xml文件---三步(1.左括号  2.根据提示选择回车   3.填写缺少的内容)
    4.logback日志框架的使用,以及感受一下控制日志的输出情况
    	4.1 拷贝相关的jar包
    	4.2 拷贝日志框架的配置文件到src路径(必须)
        4.3 编写记录日志的代码
        4.4 修改日志配置文件里面的配置,感受一下控制日志的输出情况
    5.创建多条线程---方式1,方式2,方式3
```
